# 步骤4/5：MainForm.cs —— 删除已迁往 Controller/Presenter 的 5 个大块
$ErrorActionPreference = 'Stop'
$p = 'C:\project\lunch\LlamaHarness\MainForm.cs'
$c = [System.IO.File]::ReadAllText($p)
function To-CrLf([string]$s) { $s -replace "`r?`n", "`r`n" }
$fail = 0
function DelBlock([string]$name, [string]$old) {
    $n = $script:c.Replace((To-CrLf $old), '')
    if ($n -eq $script:c) { Write-Host "[FAIL] $name 未匹配"; $script:fail++ } else { Write-Host "[OK] $name"; $script:c = $n }
}

# 块1：监控区（OnManualRefresh → ToggleRaw）
$b1 = @'
...
'@
DelBlock '块5 ApplyPhase' $b5

if ($fail -gt 0) { Write-Host '存在未匹配块，中止写回'; exit 1 }
[System.IO.File]::WriteAllText($p, $c, [System.Text.UTF8Encoding]::new($false))
Write-Host 'MainForm.cs 大块删除完成'
