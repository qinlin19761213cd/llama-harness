...
public sealed class MonitorPanelView : UserControl
{
    ...
    private static readonly Dictionary<string, string> PropsDesc = new()
    {
        ...
        ["postsamplingprobs"] = "后采样概率输出（采样后各 token 概率）",
        ["backendsampling"] = "后端采样（使用 llama.cpp 原生采样器）",
        ["nctx"] = "上下文窗口大小（token）",
        ["totalslots"] = "总槽位数（并发推理槽）",
        ["modelalias"] = "模型别名（加载时的显示名）",
        ["modelftype"] = "模型量化类型（IQ2_M - 2.7 bpw 等）",
        ["modelpath"] = "模型文件路径",
        ["vision"] = "视觉能力（多模态图片输入）",
        ["video"] = "视频输入能力",
        ["audio"] = "音频输入能力",
        ["mediamarker"] = "媒体占位标记（多模态内容替换符）",
        ["endpointslots"] = "暴露 /slots 端点（查询槽位状态）",
        ["endpointprops"] = "暴露 /props 端点（查询参数）",
        ["endpointmetrics"] = "暴露 /metrics 端点（Prometheus 指标）",
        ["ui"] = "启用内置 Web UI",
        ["supportsobjectarguments"] = "支持对象参数（arguments 为 JSON 对象）",
        ["supportsparalleltoolcalls"] = "支持并行工具调用",
        ["supportspreservereasoning"] = "支持保留思考内容",
        ["supportsreasoningeffort"] = "支持思考强度参数（reasoning_effort）",
        ["supportsstringcontent"] = "支持字符串 content",
        ["supportssystemrole"] = "支持 system 角色",
        ["supportstoolcalls"] = "支持工具调用（tool_calls）",
        ["supportstools"] = "支持 tools 声明",
        ["supportstypedcontent"] = "支持类型化 content（数组形式）",
        ["bostoken"] = "起始标记 token（BOS）",
        ["eostoken"] = "结束标记 token（EOS）",
        ["buildinfo"] = "构建版本信息（b10676-a43c3986b）",
        ["issleeping"] = "是否处于休眠状态（无请求自动休眠）",
        ["corsproxyenabled"] = "跨域代理（CORS 代理）是否启用",
    };
